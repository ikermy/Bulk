...
func TestNewDeps_WithKafkaEnvVars(t *testing.T) {
	...
	cfg.Kafka.Brokers = "localhost:9092"
	cfg.Kafka.BulkResultTopic = "bulk.result"
	// TransHistoryTopic теперь берётся из cfg, а не из os.Getenv напрямую.
	// Проверяем, что кастомное значение из конфига проходит корректно.
	cfg.Kafka.TransHistoryTopic = "trans-history.test"
